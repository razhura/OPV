...
def merge_same_materials(df):
    ...
    result_rows = []
    nama_bahan_cols = [col for col in df.columns if col.startswith('Nama Bahan Formula ')]
    indices = sorted([int(col.split()[-1]) for col in nama_bahan_cols if col.split()[-1].isdigit()])
    ...
    for row_idx in df.index:
        ...
        for idx in indices:
            ...
            if (nama_bahan_col in df.columns and 
                ...
                group_data = {
                    'original_index': idx,
                    'nama_bahan_formula': str(df.loc[row_idx, nama_bahan_col]).strip(),
                    'kode': df.loc[row_idx, f'Kode Bahan {idx}'] if f'Kode Bahan {idx}' in df.columns else '',
                    'terpakai': df.loc[row_idx, f'Kuantiti > Terpakai {idx}'] if f'Kuantiti > Terpakai {idx}' in df.columns else '',
                    'rusak': df.loc[row_idx, f'Kuantiti > Rusak {idx}'] if f'Kuantiti > Rusak {idx}' in df.columns else '',
                    'lot': df.loc[row_idx, f'No Lot Supplier {idx}'] if f'No Lot Supplier {idx}' in df.columns else '',
                    'qc': df.loc[row_idx, f'Label QC {idx}'] if f'Label QC {idx}' in df.columns else ''
        ...
        for group in materials_groups:
            seen_names[group['nama_bahan_formula']].append(group)
        ...
        for group in groups_to_move:
            new_row = pd.Series(index=df.columns, dtype=object)
            new_row[:] = ''
            target_pos = group['original_index']
            new_row[f'Nama Bahan Formula {target_pos}'] = group['nama_bahan_formula']
            new_row[f'Kode Bahan {target_pos}'] = group['kode']
            new_row[f'Kuantiti > Terpakai {target_pos}'] = group['terpakai']
            new_row[f'Kuantiti > Rusak {target_pos}'] = group['rusak']
            new_row[f'No Lot Supplier {target_pos}'] = group['lot']
            new_row[f'Label QC {target_pos}'] = group['qc']
            result_rows.append(new_row)

        # Tambahkan baris jumlah kuantiti (jika ada bahan yang muncul >1 kali)
        for nama_bahan, group_list in seen_names.items():
            if len(group_list) <= 1:
                continue

            total_terpakai = sum([
                float(str(g['terpakai']).split()[0].replace(".", "").replace(",", ".")) 
                if g['terpakai'] else 0 for g in group_list
            ])
            total_rusak = sum([
                float(str(g['rusak']).split()[0].replace(".", "").replace(",", ".")) 
                if g['rusak'] else 0 for g in group_list
            ])
            ...
            for idx in indices:
                ...
                if col in total_row.index:
                    if str(df.loc[row_idx, col]).strip() == nama_bahan:
                        total_row[col] = ''
                        total_row[f'Kuantiti > Terpakai {idx}'] = f"{int(total_terpakai):,}".replace(",", ".") + " GRAM"
                        total_row[f'Kuantiti > Rusak {idx}'] = f"{int(total_rusak):,}".replace(",", ".")
                        break
